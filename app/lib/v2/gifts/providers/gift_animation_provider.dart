...
class GiftAnimationState {
  final GiftV2SendEvent? activeLegendary;
  final GiftV2SendEvent? activeLarge;
  final List<GiftV2SendEvent?> activeMediumSlots; // length 2
  ...
  GiftAnimationState copyWith({
    GiftV2SendEvent? activeLegendary,
    bool resetLegendary = false,
    GiftV2SendEvent? activeLarge,
    bool resetLarge = false,
    List<GiftV2SendEvent?>? activeMediumSlots,
...
class GiftAnimationController extends StateNotifier<GiftAnimationState> {
  ...
  GiftV2SocketService? _socket;
  StreamSubscription? _sentSub;
  StreamSubscription? _legendarySub;

  final Queue<GiftV2SendEvent> _legendaryQueue = Queue();
  final Queue<GiftV2SendEvent> _largeQueue = Queue();
  final Queue<GiftV2SendEvent> _mediumQueue = Queue();
  ...
  void bindSocket(GiftV2SocketService socket) {
    ...
    _legendarySub = socket.legendaryIncomingStream.listen((_) {
      // Pre-warm hook — actual playback waits for gift_v2_sent.
    });
  }

  /// External entry point: any gift event from any source.
  void ingest(GiftV2SendEvent event) {
  ...
  void _enqueueLegendary(GiftV2SendEvent event) {
  ...
  void _enqueueLarge(GiftV2SendEvent event) {
  ...
  void _enqueueMedium(GiftV2SendEvent event) {
    final slots = List<GiftV2SendEvent?>.from(state.activeMediumSlots);
  ...
  void _appendSmall(GiftV2SendEvent event) {
