...
import {
  ...
  setClientSeed,
  verifySpin,
} from '../services/olympus.service';

/**
 * REST surface for بوابات أوليمبوس (Gates of Olympus). Every spin is settled
 * server-side here — the client never decides a symbol, a tumble, a lightning
 * multiplier or a payout, it only replays the sequence this endpoint hands back.
 */
...
export const getOlympusState = async (req: Request, res: Response) => {
  try {
    ...
    console.error('[olympus] state error', error);
    res.status(500).json({ success: false, message: 'تعذر تحميل اللعبة' });
  }
};

export const spinOlympus = async (req: Request, res: Response) => {
  try {
    const userId = userIdOf(req);
    if (!userId) return res.status(401).json({ success: false, message: 'Unauthorized' });
    ...
    console.error('[olympus] spin error', error);
    res.status(500).json({ success: false, message: 'تعذر تنفيذ الجولة' });
  }
};

export const getOlympusHistory = async (req: Request, res: Response) => {
  const userId = userIdOf(req);
  if (!userId) return res.status(401).json({ success: false, message: 'Unauthorized' });
  res.json({ success: true, history: getHistory(userId) });
};

export const getOlympusFairness = async (req: Request, res: Response) => {
  const userId = userIdOf(req);
  if (!userId) return res.status(401).json({ success: false, message: 'Unauthorized' });
  res.json({ success: true, fairness: await getFairness(userId) });
};

export const setOlympusClientSeed = async (req: Request, res: Response) => {
  const userId = userIdOf(req);
  if (!userId) return res.status(401).json({ success: false, message: 'Unauthorized' });
...
export const rotateOlympusSeed = async (req: Request, res: Response) => {
  const userId = userIdOf(req);
  if (!userId) return res.status(401).json({ success: false, message: 'Unauthorized' });
...
export const verifyOlympusSpin = async (req: Request, res: Response) => {
  try {
    ...
    console.error('[olympus] verify error', error);
