...
import {
  ...
  setClientSeed,
  verifySpin,
} from '../services/asterion.service';

/**
 * REST surface for أستيريون (Citadel of Asterion). Every spin is settled
 * server-side here — the client never decides a symbol, a tumble, a Storm Orb
 * or a payout, it only replays the sequence this endpoint hands back.
 */
...
export const getAsterionState = async (req: Request, res: Response) => {
  try {
    ...
    console.error('[asterion] state error', error);
    res.status(500).json({ success: false, message: 'تعذر تحميل اللعبة' });
  }
};

export const spinAsterion = async (req: Request, res: Response) => {
  try {
    const userId = userIdOf(req);
    if (!userId) return res.status(401).json({ success: false, message: 'Unauthorized' });
    ...
    console.error('[asterion] spin error', error);
    res.status(500).json({ success: false, message: 'تعذر تنفيذ الجولة' });
  }
};

export const getAsterionHistory = async (req: Request, res: Response) => {
  const userId = userIdOf(req);
  if (!userId) return res.status(401).json({ success: false, message: 'Unauthorized' });
  res.json({ success: true, history: getHistory(userId) });
};

export const getAsterionFairness = async (req: Request, res: Response) => {
  const userId = userIdOf(req);
  if (!userId) return res.status(401).json({ success: false, message: 'Unauthorized' });
  res.json({ success: true, fairness: await getFairness(userId) });
};

export const setAsterionClientSeed = async (req: Request, res: Response) => {
  const userId = userIdOf(req);
  if (!userId) return res.status(401).json({ success: false, message: 'Unauthorized' });
...
export const rotateAsterionSeed = async (req: Request, res: Response) => {
  const userId = userIdOf(req);
  if (!userId) return res.status(401).json({ success: false, message: 'Unauthorized' });
...
export const verifyAsterionSpin = async (req: Request, res: Response) => {
  try {
    ...
    console.error('[asterion] verify error', error);
