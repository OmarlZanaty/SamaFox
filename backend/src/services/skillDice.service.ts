...
const REWARD_TIERS: { minScore: number; multiplier: number }[] = [
  { minScore: 100, multiplier: Number(process.env.SKILL_DICE_EXACT_RETURN ?? 0.95) },
  { minScore: 80, multiplier: 0.7 },
  { minScore: 55, multiplier: 0.5 },
  { minScore: 30, multiplier: 0.35 },
  { minScore: 0, multiplier: Number(process.env.SKILL_DICE_MIN_RETURN ?? 0.2) },
];
...
export async function joinRound(userId: number, entry: number) {
  ...
  await grantStakeValue(userId, 'dice', entry, `round:${round.id}`);
